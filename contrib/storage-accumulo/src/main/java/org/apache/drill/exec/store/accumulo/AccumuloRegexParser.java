...
public class AccumuloRegexParser {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(org.apache.drill.exec.store.accumulo.AccumuloRegexParser.class);
  ...
  public AccumuloRegexParser(FunctionCall call) {
    this(likeString(call), escapeString(call));
  }

  public AccumuloRegexParser(String likeString) {
    this(likeString, null);
  }

  public AccumuloRegexParser(String likeString, Character escapeChar) {
   ...
   */
  public org.apache.drill.exec.store.accumulo.AccumuloRegexParser parse() {
  ...
  private static String likeString(FunctionCall call) {
    return ((QuotedString) call.args.get(1)).value.toString();
  }

  private static Character escapeString(FunctionCall call) {
    if (call.args.size() > 2) {
      return ((QuotedString) call.args.get(2)).value.toString().charAt(0);
    }
    return null;
  }
