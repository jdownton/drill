...
public abstract class AccumuloPushFilterIntoScan extends StoragePluginOptimizerRule {
  ...
  public static final StoragePluginOptimizerRule FILTER_ON_PROJECT = new AccumuloPushFilterIntoScan(RelOptHelper.some(FilterPrel.class, RelOptHelper.some(ProjectPrel.class, RelOptHelper.any(ScanPrel.class))), "HBasePushFilterIntoScan:Filter_On_Project") {
  ...
  protected void doPushFilterToScan(final RelOptRuleCall call, final FilterPrel filter, final ProjectPrel project, final ScanPrel scan, final AccumuloGroupScan groupScan, final RexNode condition) {
    ...
    final AccumuloGroupScan newGroupsScan = new AccumuloGroupScan(groupScan.getUserName(), groupScan.getStoragePlugin(),
